...
namespace geo {
  ...
  //......................................................................
  GeometryTestAlg::GeometryTestAlg(geo::GeometryCore const* geometry,
                                   geo::ChannelMapAlg const* channelMap,
                                   fhicl::ParameterSet const& pset)
    : geom{geometry}
    , channelMapAlg{channelMap}
  ...
  void GeometryTestAlg::printDetectorIntro() const
  {
    ...
    WireGeo const& testWire = geom->Wire(WireID(0, 0, 1, 10));
  ...
  void GeometryTestAlg::printWirePos()
  {
    ...
    for (unsigned int t = 0; t < std::floor(geom->NTPC(cs) / 12) + 1; ++t) {
      for (unsigned int p = 0; p < 3; ++p) {
        auto const& plane = geom->Cryostat(cs).TPC(t).Plane(p);
        for (unsigned int w = 0; w < plane.Nwires(); w++) {
  ...
  void GeometryTestAlg::printWiresInTPC(const TPCGeo& tpc, std::string indent /* = "" */) const
  {
  ...
  void GeometryTestAlg::testTPC(CryostatID const& cid)
  {
    ...
    for (auto const& tpc : geom->Iterate<TPCGeo>(cid)) {
      ...
      {
        mf::LogVerbatim log{"GeometryTest"};
        log << "\n\t\tTPC " << tpc.ID() << " " << tpc.ActiveVolume()->GetName() << " has "
            << tpc.Nplanes() << " planes."
      ...
      for (size_t p = 0; p < tpc.Nplanes(); ++p) {
        ...
        mf::LogVerbatim("GeometryTest") << "\t\t  pitch from plane 0 is " << tpc.Plane0Pitch(p);

      } // for plane
      DriftDirection_t dir = tpc.DriftDirection();
      if (dir == kNegX) {
        mf::LogVerbatim("GeometryTest")
      ...
      const unsigned int tpcNo = cryo.PositionToTPCID(worldLoc, 1 + 1.e-4).TPC;
  ...
  void GeometryTestAlg::testPlaneDirections() const
  {
    ...
    unsigned int nErrors = 0;
    for (auto const& plane : geom->Iterate<PlaneGeo>()) {
      //
  ...
  void GeometryTestAlg::testParallelWires() const
  {
    ...
    for (auto const& plane : geom->Iterate<PlaneGeo>()) {
      ...
      for (WireGeo const& wire : geom->Iterate<WireGeo>(plane.ID())) {
  ...
  void GeometryTestAlg::testWireCoordAngle() const
  {
    ...
    for (auto const& planeid : geom->Iterate<PlaneID>()) {

      PlaneGeo const& plane = geom->Plane(planeid);
      ...
      WireGeo const& middle_wire = geom->Wire(middle_wire_id);
      ...
      const double middle_coord = geom->Plane(planeid).WireCoordinate(middle_wire_center);
      ...
      const double next_coord = geom->Plane(planeid).WireCoordinate(on_next_wire);
  ...
  void GeometryTestAlg::testChannelToROP() const
  {
    ...
    try {
      readout::ROPID invalidROP = channelMapAlg->ChannelToROP(raw::InvalidChannelID);
    ...
    for (raw::ChannelID_t channel = 0; channel < channelMapAlg->Nchannels(); ++channel) {

      readout::ROPID const ropid = channelMapAlg->ChannelToROP(channel);

      auto const firstChannel = channelMapAlg->FirstChannelInROP(ropid);
      auto const lastChannel = firstChannel + channelMapAlg->Nchannels(ropid);
  ...
  void GeometryTestAlg::testChannelToWire() const
  {
    ...
    for (auto const& testWireID : geom->Iterate<WireID>()) {

      raw::ChannelID_t channel = channelMapAlg->PlaneWireToChannel(testWireID);
      ...
      auto const wireIDs = channelMapAlg->ChannelToWire(channel);
      ...
      auto const channelSigType = channelMapAlg->SignalType(channel);
      if (channelSigType != channelMapAlg->SignalType(testWireID.planeID())) {
        throw cet::exception("BadChannelLookup")
          ...
          << channelMapAlg->SignalType(testWireID.planeID()) << "\n";
      }

      auto const channelView = channelMapAlg->View(channel);
      if (channelView != geom->Plane(testWireID).View()) {
        throw cet::exception("BadChannelLookup")
          << "Geometry service claims channel #" << channel << " should be on view "
          << channelMapAlg->View(channel) << " but the plane of " << std::string(testWireID)
  ...
  void GeometryTestAlg::testFindPlaneCenters()
  {
    ...
    for (auto const& plane : geom->Iterate<PlaneGeo>(CryostatID{0})) {
      ...
      for (int iW = -nOutsides * steps; iW <= +nOutsides * steps; ++iW) {
        ...
        for (int iD = -nOutsides * steps; iD <= +nOutsides * steps; ++iD) {
          ...
          for (double distance : {-30., 0.0, +30.0}) {
            ...
            //
            const bool expected_onPlane = inWidth && inDepth;
            const bool onPlane = plane.isProjectionOnPlane(expected_point);
            if (expected_onPlane != onPlane) {
              ++nErrors;
              mf::LogProblem("GeometryTestAlg")
    ...
    for (auto const& cryostat : geom->Iterate<CryostatGeo>()) {
      for (size_t t = 0; t < cryostat.NTPC(); ++t) {
        ...
        for (size_t i = 0; i < tpc->Nplanes(); ++i) {
          const PlaneGeo* plane = &tpc->Plane(i);
          ...
          for (size_t j = 1; j < plane->Nwires(); ++j) {
            ...
            if (xyz.Z() < xyzprev.Z())
              throw cet::exception("WireOrderProblem")
                ...
                << "Cryostat " << cryostat.ID() << ", TPC " << t << ", Plane " << i << ";  at wire "
                << j << "\n";
  ...
  //......................................................................
  inline std::array<double, 3> GeometryTestAlg::GetIncreasingWireDirection(const PlaneGeo& plane)
  {
  ...
  void GeometryTestAlg::testNearestWire()
  {
    ...
    for (auto const& plane : geom->Iterate<PlaneGeo>()) {
      ...
      WireID::WireID_t wireNo = 0;
      for (WireGeo const& wire : geom->Iterate<WireGeo>(planeID)) {
        ...
        try {
          nearest = channelMapAlg->NearestChannel(wire_center, planeID);
        }
        catch (cet::exception& e) {
        ...
        try {
          wireIDs = channelMapAlg->ChannelToWire(nearest);
        ...
        if (std::find(wireIDs.begin(), wireIDs.end(), wireID) == wireIDs.end()) {
          throw cet::exception("BadPositionToChannel")
            ...
            << geom->Plane(planeID).NearestWireID(wire_center).Wire << "\nNearestChannel is "
            << nearest << "\nShould be channel " << channelMapAlg->PlaneWireToChannel(wireID);
        ...
        try {
          ...
          const double pitch = std::abs(geom->Plane(planeID).WirePitch());
          ...
          for (int i = -NSteps; i <= +NSteps; ++i) {
            ...
            if (bTestWireCoordinate) {
              try {
                wire_from_wc = geom->Plane(planeID).WireCoordinate(wire_shifted);
              }
              catch (cet::exception& e) {
            ...
            if ((expected_wire > -0.5) && (expected_wire < NWires - 0.5)) {
              ...
              try {
                wire_number_from_wc = geom->Plane(planeID).NearestWireID(wire_shifted).Wire;
              }
              catch (cet::exception& e) {
    ...
    try {
      nearest_to_what = channelMapAlg->NearestChannel(vect::toPoint(posWorld), PlaneID{0, 0, 0});
    }
    catch (const InvalidWireError& e) {
                                                       ...
                                                       Vector_t const& wireDir) const
  {
    ...
    if (isOrthogonalTo(plane.GetIncreasingWireDirection())) return true;
  ...
  void GeometryTestAlg::testWireIntersection() const
  {
    ...
    for (auto const& TPC : geom->Iterate<TPCGeo>()) {
      ...
      if (TPC.ID().Cryostat < geom->Ncryostats() - 1) {
        ...
        WireGeo const& wire1 = geom->Wire(w1);
        ...
        for (auto const& plane : geom->Iterate<PlaneGeo>(otherCryo.ID())) {
      ...
      if (TPC.ID().TPC < geom->NTPC(TPC.ID().asCryostatID()) - 1) {
        ...
        PlaneGeo const* otherPlane = nullptr;
        for (auto const& plane : geom->Iterate<PlaneGeo>(cryo.ID())) {
      ...
      for (PlaneGeo const& plane : TPC.IteratePlanes()) {
      ...
      PlaneGeo const& refPlane = TPC.SmallestPlane();
  ...
  unsigned int GeometryTestAlg::testWireIntersectionAt(const TPCGeo& TPC,
                                                       Point_t const& point) const
  {
    ...
    bool const bDriftOnX = (TPC.DriftDir() == Xaxis()) || (TPC.DriftDir() == -Xaxis());
    ...
    for (unsigned int iPlane = 0; iPlane < NPlanes; ++iPlane) {
      const PlaneGeo& plane = TPC.Plane(iPlane);
      WireCoordDirs[iPlane] = plane.GetIncreasingWireDirection();
      WirePitch[iPlane] = plane.WirePitch();

      const double WireDistance = plane.WireCoordinate(point);
    ...
    for (unsigned int iPlane1 = 0; iPlane1 < NPlanes; ++iPlane1) {
      ...
      const WireID& w1 = WireIDs[iPlane1];
      PlaneGeo const& plane1 = TPC.Plane(w1);
      WireGeo const& w1obj = plane1.Wire(w1);

      for (unsigned int iPlane2 = iPlane1 + 1; iPlane2 < NPlanes; ++iPlane2) {
        const WireID& w2 = WireIDs[iPlane2];
        PlaneGeo const& plane2 = TPC.Plane(w2);
        ...
        if (bDriftOnX) { // legacy code check
          ...
          auto widIntersect = geom->WireIDsIntersect(w1, w2);
          if (!widIntersect) {
            MF_LOG_ERROR("GeometryTest")
  ...
  void GeometryTestAlg::testThirdPlane() const
  {
    ...
    unsigned int nErrors = 0;
    for (auto const& tpcid : geom->Iterate<TPCID>()) {
      const TPCGeo& TPC = geom->TPC(tpcid);
  ...
  void GeometryTestAlg::testThirdPlane_dTdW() const
  {
    ...
    unsigned int nErrors = 0;
    for (TPCID const& tpcid : geom->Iterate<TPCID>()) {
      const TPCGeo& TPC = geom->TPC(tpcid);

      const double driftVelocity = 0.1 * ((TPC.DriftDirection() == kNegX) ? -1. : +1);
      ...
      const Point_t A{TPC.CenterX(), TPC.CenterY(), TPC.CenterZ()};
      // pick a radius half the way to the closest border
      const double radius = std::min({TPC.HalfWidth(), TPC.HalfHeight(), TPC.Length() / 2.}) / 2.;
    ...
    ) const
  {
    ...
    TPCGeo const& TPC = geom->TPC(tpcid);
    ...
    default:
      throw cet::exception("InternalError")
        << "GeometryTestAlg::ExpectedPlane_dTdW(): drift direction #" << ((int)TPC.DriftDirection())
        << " of " << std::string(tpcid) << " not supported.\n";
    ...
    for (PlaneID::PlaneID_t iPlane = 0; iPlane < nPlanes; ++iPlane) {
      ...
      auto const& plane = geom->Plane(pid);
      const double wA = plane.WireCoordinate(A);
      const double wB = plane.WireCoordinate(B);
  ...
  void GeometryTestAlg::testWirePitch()
  {
    ...
    for (PlaneID const& planeid : geom->Iterate<PlaneID>()) {
  ...
  void GeometryTestAlg::testStepping()
  {
    ...
    auto const xyz_orig = geom->Plane(planeid).Wire(0).toWorldCoords(xyzWire);
    auto const dxyz_orig = geom->Plane(planeid).Wire(0).toWorldCoords(dxyzWire);
  ...
  bool GeometryTestAlg::CheckAuxDetAtPosition(Point_t const pos, unsigned int expected) const
  {
    ...
    try {
      foundDet = channelMapAlg->FindAuxDetAtPosition(pos);
    }
    catch (cet::exception const& e) {
      mf::LogProblem("GeometryTestAlg")
                                                       ...
                                                       unsigned int expectedSens) const
  {
    ...
    try {
      channelMapAlg->FindAuxDetSensitiveAtPosition(pos, foundDet, foundSensDet);
    }
    catch (cet::exception const& e) {
      mf::LogProblem("GeometryTestAlg")
