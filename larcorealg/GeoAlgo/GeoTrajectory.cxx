...
namespace geoalgo {
  ...
  double Trajectory::Length(size_t start_step, size_t end_step) const
  {
    ...
    if (end_step == 0)
      end_step = size() - 1; // By default end_step is 0. Then consider the whole trajectory()
  ...
  void Trajectory::compat(Point_t const& obj) const
  {
    ...
    if ((*(this->begin())).size() != obj.size()) {
      ...
      msg << "<<" << __FUNCTION__ << ">>"
          << " size mismatch: " << (*(this->begin())).size() << " != " << obj.size() << std::endl;
  ...
  void Trajectory::compat(Trajectory const& obj) const
  {
    ...
    if ((*(this->begin())).size() != (*obj.begin()).size()) {
      ...
      std::ostringstream msg;
      msg << "<<" << __FUNCTION__ << ">>"
          << " size mismatch: " << (*(this->begin())).size() << " != " << (*obj.begin()).size()
          << std::endl;
      throw GeoAlgoException(msg.str());
    }
  }
